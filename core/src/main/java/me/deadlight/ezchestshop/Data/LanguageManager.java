...
public class LanguageManager {
  ...
  public static List<String> getFoundlanguages() {
    File dir = new File(
      EzChestShop.getPlugin().getDataFolder() + File.separator + "translations"
    );
    return Arrays
      .asList(dir.listFiles())
      .stream()
      .map(f -> f.getName())
  ...
  public static void loadLanguages() {
    for (String locale : LanguageManager.getSupportedLanguages()) {
      File customConfigFile = new File(
        EzChestShop.getPlugin().getDataFolder() +
        File.separator +
        "translations",
        locale + ".yml"
      );
      if (!customConfigFile.exists()) {
        EzChestShop.logConsole(
          "&c[&eEzChestShop&c] &eGenerating " + locale + " file..."
        );
        customConfigFile.getParentFile().mkdirs();
        EzChestShop
          .getPlugin()
          .saveResource("translations/" + locale + ".yml", false);
      }
    }
    File customConfigFile = new File(
      EzChestShop.getPlugin().getDataFolder() + File.separator + "translations",
    ...
    EzChestShop.logConsole(
      "&c[&eEzChestShop&c] &e" + Config.language + " successfully loaded"
    );
  }
    ...
    for (String local : getSupportedLanguages()) {
      ...
      FileConfiguration fc_internal = YamlConfiguration.loadConfiguration(
        new InputStreamReader(
          EzChestShop.getPlugin().getResource("translations/" + local + ".yml")
        )
      );
  ...
  public String initialBuyPrice(double price) {
    return Utils.colorify(
      getString("shop-gui.initialbuyprice")
        .replace("%buyprice%", Utils.formatNumber(price, Utils.FormatType.GUI))
  ...
  public String initialSellPrice(double price) {
    return Utils.colorify(
      getString("shop-gui.initialsellprice")
        .replace("%sellprice%", Utils.formatNumber(price, Utils.FormatType.GUI))
        .replace("%currency%", Config.currency)
    );
  }/**
   * @return returns buy price of GUI item in lore
  ...
  public String guiAdminTitle(String shopowner) {
    return Utils.colorify(
      getString("shop-gui.admin-title")
        .replace("%shopowner%", String.valueOf(shopowner))
    );
  }

  public String guiNonOwnerTitle(String shopowner) {
    return Utils.colorify(
      getString("shop-gui.nonowner-title")
        .replace("%shopowner%", String.valueOf(shopowner))
    );
  }
  ...
  public String guiOwnerTitle(String shopowner) {
    return Utils.colorify(
      getString("shop-gui.owner-title")
        .replace("%shopowner%", String.valueOf(shopowner))
    );
  }
  ...
  public List<String> buttonSell1Lore(double price) {
    return getList("shop-gui.buttons.sellone-lore")
      .stream()
      .map(s ->
        Utils.colorify(
          s
            .replace("%price%", Utils.formatNumber(price, Utils.FormatType.GUI))
            .replace("%currency%", Config.currency)
        )
      )
      .collect(Collectors.toList());
  ...
  public List<String> buttonSell64Lore(double price) {
    return getList("shop-gui.buttons.sell64-lore")
      .stream()
      .map(s ->
        Utils.colorify(
          s
  ...
  public List<String> buttonBuy1Lore(double price) {
    return getList("shop-gui.buttons.buyone-lore")
      .stream()
      .map(s ->
        Utils.colorify(
          s
  ...
  public List<String> buttonBuy64Lore(double price) {
    return getList("shop-gui.buttons.buy64-lore")
      .stream()
      .map(s ->
        Utils.colorify(
          s
  ...
  public List<String> disabledButtonLore() {
    return getList("shop-gui.buttons.disabled-lore")
      .stream()
  ...
  public String transactionPaperTitleBuy(String name) {
    return Utils.colorify(
      getString("transactions.PaperTitleBuy").replace("%player%", name)
    );
  }

  public String transactionPaperTitleSell(String name) {
    return Utils.colorify(
      getString("transactions.PaperTitleSell").replace("%player%", name)
    );
  }

  public List<String> transactionPaperLoreBuy(
    String price,
    int count,
    String time
  ) {
    return getList("transactions.PaperLoreBuy")
      .stream()
      .map(s ->
        Utils.colorify(
          s
            .replace(
              "%price%",
              Utils.formatNumber(Double.valueOf(price), Utils.FormatType.GUI)
            )
            .replace("%currency%", Config.currency)
  ...
  public List<String> transactionPaperLoreSell(
    String price,
    int count,
    String time
  ) {
    return getList("transactions.PaperLoreSell")
      .stream()
      .map(s ->
        Utils.colorify(
          s
  ...
  public String minutesago(long minutes) {
    return Utils.colorify(
      getString("transactions.minutesago")
  ...
  public String hoursago(long hours) {
    return Utils.colorify(
      getString("transactions.hoursago")
  ...
  public String daysago(long days) {
    return Utils.colorify(
      getString("transactions.daysago").replace("%days%", String.valueOf(days))
    );
  }

  public String transactionBuyInform(
    String player,
    int amount,
    String item,
    double price
    ...
    return Utils.colorify(
      getString("transactions.player-inform-buy")
  ...
  public String transactionSellInform(
    String player,
    int amount,
    String item,
    double price
    ...
    return Utils.colorify(
      getString("transactions.player-inform-sell")
        .replace("%player%", player)
        .replace("%amount%", String.valueOf(amount))
        .replace("%item%", item)
        .replace("%price%", String.valueOf(price))
  ...
  public String toggleTransactionMessageButton() {
    return Utils.colorify(
      getString("settings.buttons.toggleTransactions.Title")
    );
  }

  public List<String> toggleTransactionMessageButtonLore(String status) {
    return getList("settings.buttons.toggleTransactions.Lore")
      .stream()
      .map(s -> Utils.colorify(s.replace("%status%", status)))
      .collect(Collectors.toList());
  }
  ...
  public List<String> disableBuyingButtonLore(String status) {
    return getList("settings.buttons.disableBuying.Lore")
      .stream()
      .map(s -> Utils.colorify(s.replace("%status%", status)))
      .collect(Collectors.toList());
  }
  ...
  public List<String> disableSellingButtonLore(String status) {
    return getList("settings.buttons.disableSelling.Lore")
      .stream()
      .map(s -> Utils.colorify(s.replace("%status%", status)))
      .collect(Collectors.toList());
  }
  ...
  public String nobodyStatusAdmins() {
    return Utils.colorify(
      getString("settings.buttons.shopAdmins.nobodyStatusAdmins")
    );
  }

  public List<String> shopAdminsButtonLore(String admins) {
    return getList("settings.buttons.shopAdmins.Lore")
      .stream()
      .map(s -> Utils.colorify(s.replace("%admins%", admins)))
      .collect(Collectors.toList());
  }
  ...
  public List<String> shareIncomeButtonLore(String admins) {
    return getList("settings.buttons.sharedIncome.Lore")
      .stream()
      .map(s -> Utils.colorify(s.replace("%status%", admins)))
      .collect(Collectors.toList());
  }
  ...
  public List<String> rotateHologramButtonLore(String rotation) {
    return getList("settings.buttons.hologramRotation.Lore")
      .stream()
      .map(s ->
        Utils.colorify(
          s
            .replace("%rotations%", formatRotations(rotation))
  ...
  public String backToShopGuiButton() {
    return Utils.colorify(
      getString("settings.buttons.other.backToShopGuiButton")
    );
  }

  public String latestTransactionsTitle() {
    return Utils.colorify(
      getString("settings.buttons.other.latestTransactionsTitle")
    );
  }
  ...
  public String toggleTransactionMessageOnInChat() {
    return Utils.colorify(
      getString("settings.chat.toggleTransactions.MessageOn")
    );
  }
  ...
  public String addingAdminWaiting() {
    return Utils.colorify(
      getString("settings.chat.shopAdmins.addingAdminWaiting")
    );
  }

  public String removingAdminWaiting() {
    return Utils.colorify(
      getString("settings.chat.shopAdmins.removingAdminWaiting")
    );
  }
  ...
  public String sucAdminAdded(String player) {
    return Utils.colorify(
      getString("settings.chat.shopAdmins.sucAdminAdded")
        .replace("%player%", player)
    );
  }
  ...
  public String sucAdminRemoved(String player) {
    return Utils.colorify(
      getString("settings.chat.shopAdmins.sucAdminRemoved")
        .replace("%player%", player)
    );
  }
  ...
  public BaseComponent[] copiedShopSettings(String hover) {
    return MineDown.parse(
      getString("settings.chat.copy-paste.copiedShopSettings")
        .replace("%settings%", hover)
    );
  }

  public String pastedShopSettings() {
    return Utils.colorify(
      getString("settings.chat.copy-paste.pastedShopSettings")
    );
  }
  ...
  public String rotateHologramInChat(String rotation) {
    return Utils.colorify(
      getString("settings.chat.hologramRotation.rotateHologramInChat")
        .replace("%rotation%", rotationFromData(rotation))
    );
  }
    ...
    for (String option : Utils.rotations) {
      if (option.equalsIgnoreCase(rotation)) {
        result +=
          ...
          "" +
          ChatColor.UNDERLINE +
          ChatColor.stripColor(rotationFromData(option)) +
          ChatColor.RESET +
          " ";
      } else {
  ...
  public String rotationUp() {
    return Utils.colorify(
      getString("settings.chat.hologramRotation.rotation.Up")
    );
  }

  public String rotationNorth() {
    return Utils.colorify(
      getString("settings.chat.hologramRotation.rotation.North")
    );
  }

  public String rotationEast() {
    return Utils.colorify(
      getString("settings.chat.hologramRotation.rotation.East")
    );
  }

  public String rotationSouth() {
    return Utils.colorify(
      getString("settings.chat.hologramRotation.rotation.South")
    );
  }

  public String rotationWest() {
    return Utils.colorify(
      getString("settings.chat.hologramRotation.rotation.West")
    );
  }

  public String rotationDown() {
    return Utils.colorify(
      getString("settings.chat.hologramRotation.rotation.Down")
    );
  }
  ...
  public String customAmountSignTitle() {
    return Utils.colorify(
      getString("customBuySell.gui-customAmountSign-title")
    );
  }

  public List<String> customAmountSignLore(
    String possibleBuyAmount,
    String possibleSellAmount
  ) {
    return getList("customBuySell.gui-customAmountSign-lore")
      .stream()
      .map(s ->
        Utils.colorify(
          s
            .replace("%buycount%", possibleBuyAmount)
            .replace("%sellcount%", possibleSellAmount)
        )
      )
      .collect(Collectors.toList());
  }
  ...
  public List<String> signEditorGuiBuy(String max) {
    ...
    return list
      .stream()
      .limit(4)
      .map(s -> Utils.colorify(s.replace("%max%", max)))
      .collect(Collectors.toList());
  }
  ...
  public String messageSuccBuy(double price) {
    return Utils.colorify(
      getString("shop-messages.successful-buy")
  ...
  public String messageSuccSell(double price) {
    return Utils.colorify(
      getString("shop-messages.successful-sell")
        .replace("%price%", Utils.formatNumber(price, Utils.FormatType.CHAT))
        .replace("%currency%", Config.currency)
    );
  }
  ...
  public BaseComponent[] cmdHelp() {
    return MineDown.parse(
      new ArrayList<>(getList("command-messages.help"))
        .stream()
        .map(s -> Utils.colorify(s))
  ...
  public BaseComponent[] cmdadminviewHelp() {
    return MineDown.parse(
      new ArrayList<>(getList("command-messages.help-admin-view-addition"))
        .stream()
        .map(s -> Utils.colorify(s))
        .collect(Collectors.joining("\n"))
    );
  ...
  public BaseComponent[] cmdadminHelp() {
    return MineDown.parse(
      new ArrayList<>(getList("command-messages.adminhelp"))
        .stream()
        .map(s -> Utils.colorify(s))
        .collect(Collectors.joining("\n"))
  ...
  public String maxShopLimitReached(int max) {
    return Utils
      .colorify(getString("command-messages.maxShopLimitReached"))
      .replace("%shoplimit%", "" + max);
  }

  public String slimeFunBlockNotSupported() {
    return Utils.colorify(
      getString("command-messages.slimeFunBlockNotSupported")
    );
  }

  public String buyGreaterThanSellRequired() {
    return Utils.colorify(
      getString("command-messages.buypriceGreaterThanSellRequired")
    );
  }

  //checkprofits.
  public BaseComponent[] checkProfitsLandingpage(
    Player player,
    double buyCost,
    int buyAmount,
    double sellCost,
    ...
    return MineDown.parse(
      getList("checkprofits.landing-menu")
        .stream()
        .collect(Collectors.joining("\n"))
        .replace("%currency%", Config.currency)
        .replace(
  ...
  public BaseComponent[] checkProfitsDetailpage(
    Player player,
    List<CheckProfitEntry> checkProfitEntries,
    int page,
    int pages
  ) {
    ...
    compb
      .append(
        MineDown.parse(
          getList("checkprofits.details-menu.header")
            .stream()
            .collect(Collectors.joining("\n"))
        )
      )
    ...
    for (int i = 0; i < Config.command_checkprofit_lines_pp; i++) {
      ...
      CheckProfitEntry checkProfitEntry = checkProfitEntries.get(
        i + ((page - 1) * Config.command_checkprofit_lines_pp)
      );
      //EzChestShop.logDebug(Utils.getFinalItemName(checkProfitEntry.getItem()) + ": " + Utils.encodeItem(checkProfitEntry.getItem()));
      String[] details = getList("checkprofits.details-menu.content")
        .stream()
        .collect(Collectors.joining("\n"))
        .split("%item%");
      for (int j = 0; j < details.length; j++) {
        compb.append(
          MineDown.parse(
            details[j].replace("%currency%", Config.currency)
        ...
        if (details.length - 1 != j) {
          compb
            .append(Utils.getFinalItemName(checkProfitEntry.getItem()))
            .event(
              new HoverEvent(
                HoverEvent.Action.SHOW_ITEM,
                new BaseComponent[] {
                  new TextComponent(
                    Utils.ItemToTextCompoundString(checkProfitEntry.getItem())
                  ),
                }
              )
            );
    ...
    compb.append(
      MineDown.parse(
        getList("checkprofits.details-menu.footer")
          .stream()
          .map(s -> {
  ...
  public String getButtonNext() {
    return Utils.colorify(
      getString("checkprofits.details-menu.hover-extra.button-next")
    );
  }

  public String getButtonPrevious() {
    return Utils.colorify(
      getString("checkprofits.details-menu.hover-extra.button-previous")
    );
  }

  public BaseComponent[] confirmProfitClear() {
    return MineDown.parse(
      getList("checkprofits.confirm-clear")
        .stream()
        .collect(Collectors.joining("\n"))
    );
  }
  ...
  public BaseComponent[] joinProfitNotification() {
    return MineDown.parse(
      getList("checkprofits.join-notification")
        .stream()
        .collect(Collectors.joining("\n"))
    );
  }
  ...
  public List<String> shulkerboxLore(
    String owner,
    String item,
    double buy,
    double sell
  ) {
    List<String> list = getList("shulkershop-dropped-lore")
      .stream()
      .map(s ->
        Utils
          .colorify(s)
          ...
          .replace(
            "%sell_price%",
            Utils.formatNumber(sell, Utils.FormatType.GUI)
          )
          .replace("%currency%", Config.currency)
      )
      .collect(Collectors.toList());
    ...
    return new ComponentBuilder("")
      .append(
        TextComponent.fromLegacyText(
          Utils.colorify(
            getString("other.update-notifications")
              .replace("%current_version%", curV)
              .replace("%new_version%", newV)
          )
        )
      )
      .event(
      ...
      .event(
        new HoverEvent(
          ...
          TextComponent.fromLegacyText(
            Utils.colorify("&cClick to check out the Spigot Page!")
          )
        )
      )
      .create();
