...
public abstract class Database {
  ...
  public void prepareColumn(String table, String primary_key, String key) {
    ...
    try {
      ...
      ps =
        conn.prepareStatement(
          "REPLACE INTO " + table + " (" + primary_key + ") VALUES(?)"
        );
   ...
   */
  public List<String> getKeysByExistance(
    String primary_key,
    String column,
    String table
  ) {
   ...
   */
  public List<String> getKeysByExpresiion(
    String primary_key,
    String column,
    String table,
    String expression
  ...
  public List<String> getKeys(String primary_key, String table) {
    ...
    try {
      conn = getSQLConnection();
      ps =
        conn.prepareStatement("SELECT " + primary_key + " FROM " + table + ";");
  ...
  public HashMap<Location, EzShop> queryShops() {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        map.put(
          Utils.StringtoLocation(sloc),
          new EzShop(
            Utils.StringtoLocation(sloc),
            rs.getString("owner"),
            Utils.decodeItem(rs.getString("item")),
            rs.getDouble("buyPrice"),
            rs.getDouble("sellPrice"),
            new ShopSettings(
              sloc,
              rs.getBoolean("msgToggle"),
              rs.getBoolean("buyDisabled"),
              rs.getBoolean("sellDisabled"),
              rs.getString("admins"),
              rs.getBoolean("shareIncome"),
              rs.getString("transactions"),
              rs.getBoolean("adminshop"),
