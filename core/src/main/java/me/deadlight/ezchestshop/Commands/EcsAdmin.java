...
public class EcsAdmin implements CommandExecutor, TabCompleter {
  ...
  public boolean onCommand(
    ...
    if (sender instanceof Player) {
      ...
      if (
        ...
        player.hasPermission("ecs.admin.remove") ||
        player.hasPermission("ecs.admin.reload") ||
        player.hasPermission("ecs.admin.create")
      ) {
        if (size == 0) {
          sendHelp(player);
        } else {
          ...
          if (
            ...
            firstarg.equalsIgnoreCase("reload") &&
            (
              player.hasPermission("ecs.admin.reload") ||
              player.hasPermission("ecs.admin")
            )
            ...
            player.sendMessage(
              Utils.colorify("&aEzChestShop successfully reloaded!")
            );
          } else if (
            firstarg.equalsIgnoreCase("create") &&
            (
              player.hasPermission("ecs.admin.create") ||
              player.hasPermission("ecs.admin")
            )
  ...
  public List<String> onTabComplete(
    ...
    List<String> list_firstarg = Arrays.asList(
      "create",
      "reload",
      "remove",
      "help"
    ...
    if (sender instanceof Player) {
      ...
      if (
        p.hasPermission("ecs.admin") ||
        p.hasPermission("ecs.admin.reload") ||
        p.hasPermission("ecs.admin.create") ||
        p.hasPermission("ecs.admin.remove")
      ) {
  ...
  private void createShop(Player player, String[] args, Block target)
    ...
    if (target != null && target.getType() != Material.AIR) {
      ...
      if (EzChestShop.worldguard) {
        if (
          !WorldGuardUtils.queryStateFlag(
            FlagRegistry.CREATE_ADMIN_SHOP,
            player
          )
        ) {
          player.sendMessage(lm.notAllowedToCreateOrRemove());
