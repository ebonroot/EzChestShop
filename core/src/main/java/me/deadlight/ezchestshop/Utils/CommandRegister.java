...
public class CommandRegister {
  ...
  public boolean registerCommandAlias(Command command, String alias)
    throws CommandFetchException {
  ...
  private SimpleCommandMap getSimpleCommandMap() throws CommandFetchException {
    try {
      return (SimpleCommandMap) getCommandMap(
        EzChestShop.getPlugin().getServer()
      );
    } catch (Exception e) {
      e.printStackTrace();
  ...
  private Map<String, Command> getKnownCommands() throws CommandFetchException {
    try {
      ...
      Field field =
        this.getServerField("knownCommands", SimpleCommandMap.class);
  ...
  private Field getServerField(String name, Class<?> clazz)
