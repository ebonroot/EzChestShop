  ...
  public BaseComponent[] checkProfitsDetailpage(
    ...
    for (int i = 0; i < Config.command_checkprofit_lines_pp; i++) {
      ...
      compb
        .append(
          MineDown.parse(
            getList("checkprofits.details-menu.content")
              .stream()
              .collect(Collectors.joining("\n"))
              .replace("%currency%", Config.currency)
              ...
              .replace(
                "%item%",
                "[" +
                Utils.getFinalItemName(checkProfitEntry.getItem()) +
                "]" +
                "(show_item=" +
                NBTItem
                  .convertItemtoNBT(checkProfitEntry.getItem())
                  .getString("id") +
                " " +
                NBTItem
                  .convertItemtoNBT(checkProfitEntry.getItem())
                  .toString()
                  .replaceAll("(\\{id:.*,tag:)|,Count:.*b}", "") +
                ")"
              )
          ),
