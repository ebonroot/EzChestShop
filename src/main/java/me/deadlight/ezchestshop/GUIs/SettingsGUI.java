...
public class SettingsGUI {
  ...
  public void showGUI(Player player, Block containerBlock, boolean isAdmin) {
    ...
    //Message Toggle Item
    boolean isToggleMessageOn =
      dataContainer.get(
        new NamespacedKey(EzChestShop.getPlugin(), "msgtoggle"),
        PersistentDataType.INTEGER
      ) ==
      1;
    ItemStack messageToggleItem = new ItemStack(
      grayGreenChooser(isToggleMessageOn),
      1
    );
    ...
    GuiItem messageToggle = new GuiItem(
      messageToggleItem,
      event -> {
        event.setCancelled(true);
        ...
        if (checkIfOn(event.getCurrentItem().getType())) {
          ...
          ShopContainer
            .getShopSettings(containerBlock.getLocation())
            .setMsgtoggle(false);
        } else {
          TileState state = ((TileState) containerBlock.getState());
          state
    ...
    //Message Toggle Item
    boolean isBuyDisabled =
      dataContainer.get(
        new NamespacedKey(EzChestShop.getPlugin(), "dbuy"),
        PersistentDataType.INTEGER
      ) ==
      1;
    ItemStack buyDisabledItem = new ItemStack(
      grayGreenChooser(isBuyDisabled),
      1
    );
    ...
    GuiItem buyDisabled = new GuiItem(
      buyDisabledItem,
      event -> {
        event.setCancelled(true);
    ...
    boolean isSellDisabled =
      dataContainer.get(
        new NamespacedKey(EzChestShop.getPlugin(), "dsell"),
        PersistentDataType.INTEGER
      ) ==
      1;
    ItemStack sellDisabledItem = new ItemStack(
      grayGreenChooser(isSellDisabled),
      1
    );
    ...
    GuiItem sellDisabled = new GuiItem(
      sellDisabledItem,
      event -> {
        event.setCancelled(true);
    ...
    if (!isAdmin) {
      boolean hastAtLeastOneAdmin = !dataContainer
        .get(
      ...
      boolean isSharedIncome =
        ...
        1;
      ItemStack sharedIncomeItem = new ItemStack(
        grayGreenChooser(isSharedIncome),
        1
      );
      ...
      GuiItem sharedIncome = new GuiItem(
        sharedIncomeItem,
        event -> {
          event.setCancelled(true);
    ...
    String rotation = dataContainer.get(
    ...
    GuiItem rotationItem = new GuiItem(
      rotationItemStack,
      event -> {
        event.setCancelled(true);
    ...
    GuiItem backItem = new GuiItem(
      ...
      event -> {
        event.setCancelled(true);
        String owneruuid = dataContainer.get(
        ...
        if (
          player.getUniqueId().toString().equalsIgnoreCase(owneruuid) || isAdmin
        ) {
  ...
  private List<String> shareIncomeLoreChooser(
    boolean data,
    LanguageManager lm
  ) {
    List<String> lores;
  ...
  private List<String> signLoreChooser(
    boolean data,
